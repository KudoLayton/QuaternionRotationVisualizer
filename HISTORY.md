...
## v2.4 - 3D 화살표 애니메이션 및 실수 성분(w) 관찰

### 순수 쿼터니언과 실수 성분 변화 시각화
V는 **순수 쿼터니언** (w=0, 실수 성분 없음)
QV 연산 후 w≠0 (실수 성분이 생김!)
QVQ⁻¹ 연산 후 w≈0 (실수 성분이 다시 사라짐 → 순수 쿼터니언으로 복귀)
...
- V의 3D 위치 (w=0, 순수 쿼터니언)
- QV의 3D 위치 (w≠0, 순수 쿼터니언 아님!)
- 주황색 화살표로 표시
- 실수 성분(w) 변화를 console에 출력

### animateRealQVQ() 3D 애니메이션
- Phase 2: V → QV (w=0 → w≠0, 순수 쿼터니언 아니게 됨, 주황색)
- Phase 3: QV → QVQ⁻¹ (w≠0 → w≈0, 다시 순수 쿼터니언, 주황→녹색)
- 순수 회전의 특성: 최종 결과의 실수 성분 w ≈ 0

### 핵심 통찰
```
V (w=0) → QV (w≠0) → QVQ⁻¹ (w≈0)
```
- V는 순수 쿼터니언 (실수부=0)
- QV 곱셈: 실수 성분이 생성됨 (더 이상 순수 쿼터니언이 아님!)
- ×Q⁻¹ 곱셈: 실수 성분이 다시 소멸 (순수 쿼터니언으로 복귀)
- 이것이 바로 "순수 회전"의 수학적 증거: 순수 쿼터니언을 회전시키면 다시 순수 쿼터니언!
